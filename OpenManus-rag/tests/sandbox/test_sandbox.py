...
async def test_sandbox_network_access(sandbox):
    ...
    # Test HTTPS connectivity with the standard library already present in the
    # configured Python image. Network access, not live package installation,
    # is the sandbox contract under test.
    result = await sandbox.terminal.run_command(
        "python3 -c \"import urllib.request; "
        "response = urllib.request.urlopen('https://www.example.com', timeout=30); "
