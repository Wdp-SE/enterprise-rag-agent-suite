...
class TextSplitter():
    ...
    def _split_report(self, file_content: Dict[str, any], serialized_tables_report_path: Optional[Path] = None) -> Dict[str, any]:
        ...
        file_content['metainfo'] = normalize_document_metadata(file_content.get('metainfo'))
        ...
        for page in file_content['content']['pages']:
            ...
            # ``page`` remains the legacy physical-page field used by retrieval
            # and citation.  ``page_number`` makes that invariant explicit for
            # OCR-derived pages without changing the existing data contract.
            page.setdefault('page_number', page['page'])
            # 普通文本分块
            page_chunks = self._split_page(page)
            for chunk in page_chunks:
                chunk['id'] = chunk_id
                chunk['chunk_id'] = f"{document_id}:{chunk_id}"
            ...
            if tables_by_page and page['page'] in tables_by_page:
                for table in tables_by_page[page['page']]:
                    table['id'] = chunk_id
                    table['chunk_id'] = f"{document_id}:{chunk_id}"
                    table['document_id'] = document_id
                    table.setdefault('page_number', table['page'])
                    for field in OCR_PAGE_METADATA_FIELDS:
                        if field in page:
                            table[field] = page[field]
    ...
    def _split_page(self, page: Dict[str, any], chunk_size: int = 300, chunk_overlap: int = 50) -> List[Dict[str, any]]:
        ...
        for chunk in chunks:
            chunk_with_meta = {
                "page": page['page'],
                "page_number": page.get('page_number', page['page']),
                "length_tokens": self.count_tokens(chunk),
                "text": chunk
            }
            for field in OCR_PAGE_METADATA_FIELDS:
                if field in page:
                    chunk_with_meta[field] = page[field]
