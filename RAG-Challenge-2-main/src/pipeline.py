...
class RunConfig:
    ...
    section_aware_chunking: bool = False
    ...
    version_manifest_path: Optional[str] = None
    historical_vector_db_dir: Optional[str] = None
    historical_documents_dir: Optional[str] = None
    version_as_of_date: Optional[str] = None
    ...
    def __post_init__(self):
        if self.routing_mode not in {"legacy_company", "generic"}:
            raise ValueError("routing_mode must be 'legacy_company' or 'generic'")
        if self.routing_mode == "generic" and self.full_context:
            raise ValueError("full_context is disabled in generic routing mode")
        if self.routing_mode == "generic":
            self.submission_file = False
        if self.retrieval_mode not in {"generic_dense", "rd_hybrid"}:
            raise ValueError("retrieval_mode must be 'generic_dense' or 'rd_hybrid'")
        if self.retrieval_mode == "rd_hybrid" and self.routing_mode != "generic":
            raise ValueError("rd_hybrid retrieval requires routing_mode='generic'")
        self.api_provider, self.answering_model = resolve_provider_model(
            self.api_provider, self.answering_model, "answer"
        )
        self.embedding_provider, self.embedding_model = resolve_provider_model(
            self.embedding_provider, self.embedding_model, "embedding"
        )
        self.rerank_provider, self.rerank_model = resolve_provider_model(
            self.rerank_provider, self.rerank_model, "rerank"
        )
        if self.trusted_qa_mode not in {"OFF", "SHADOW", "ENFORCE"}:
            raise ValueError("trusted_qa_mode must be OFF, SHADOW, or ENFORCE")
        if self.trusted_qa_enforcement_scope != "POST_ANSWER_ONLY":
...
class Pipeline:
    ...
    def parse_pdf_reports_sequential(self):
        ...
        pdf_parser = PDFParser(
            ...
            csv_metadata_path=(
                self.paths.subset_path if self.paths.subset_path.exists() else None
            )
        )
    ...
    def chunk_reports(self, include_serialized_tables: bool = False):
        ...
        text_splitter = (
            SectionAwareTextSplitter(
                child_chunk_size=self.run_config.child_chunk_size,
                child_chunk_overlap=self.run_config.child_chunk_overlap,
            )
            if self.run_config.section_aware_chunking
            else TextSplitter()
        )
    ...
    def process_parsed_reports(self):
        ...
        if self.run_config.use_bm25_db:
            print("Step 5: Creating BM25 databases...")
            self.create_bm25_db()
    ...
    def process_questions(self):
        processor = QuestionsProcessor(
            ...
            embedding_provider=self.run_config.embedding_provider,
            embedding_model=self.run_config.embedding_model,
            rerank_provider=self.run_config.rerank_provider,
            rerank_model=self.run_config.rerank_model,
            routing_mode=self.run_config.routing_mode,
            version_governance_enabled=self.run_config.version_governance_enabled,
            version_manifest_path=self.run_config.version_manifest_path,
            historical_vector_db_dir=self.run_config.historical_vector_db_dir,
            historical_documents_dir=self.run_config.historical_documents_dir,
            version_as_of_date=self.run_config.version_as_of_date,
            trusted_qa_mode=self.run_config.trusted_qa_mode,
            trusted_qa_policy_profile=self.run_config.trusted_qa_policy_profile,
            trusted_qa_enforcement_scope=self.run_config.trusted_qa_enforcement_scope,
            retrieval_mode=self.run_config.retrieval_mode,
            bm25_db_dir=self.paths.bm25_db_path,
            dense_top_k=self.run_config.dense_top_k,
            bm25_top_k=self.run_config.bm25_top_k,
            fusion_top_k=self.run_config.fusion_top_k,
            rerank_top_k=self.run_config.rerank_top_k,
            rrf_k=self.run_config.rrf_k,
            dense_weight=self.run_config.dense_weight,
            bm25_weight=self.run_config.bm25_weight,
            neighbor_children=self.run_config.neighbor_children,
            context_max_tokens=self.run_config.context_max_tokens,
        )
...
generic_config = RunConfig(
    ...
    parallel_requests=1,
    submission_file=False,
    submission_name="Generic enterprise document RAG",
    pipeline_details="Generic corpus retrieval + Parent Document Retrieval + reranking + structured answer",
    config_suffix="_generic",
)

rd_hybrid_config = RunConfig(
    routing_mode="generic",
    retrieval_mode="rd_hybrid",
    section_aware_chunking=True,
    child_chunk_size=180,
    ...
    parallel_requests=1,
    submission_file=False,
    submission_name="R&D Document RAG V2 prototype",
    pipeline_details="Section-aware child retrieval + Dense/BM25 RRF + rerank + bounded section context",
    config_suffix="_rd_hybrid",
)
...
max_config = RunConfig(
    ...
    rerank_model="gpt-4o-mini-2024-07-18",
    config_suffix="_max"
)
