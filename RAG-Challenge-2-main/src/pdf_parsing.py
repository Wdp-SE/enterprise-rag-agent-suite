...
class PDFParser:
    ...
    def _parse_csv_metadata(csv_path: Path) -> dict:
        """Parse legacy or generic document metadata keyed by the source file stem."""
        import csv
        metadata_lookup = {}
        ...
        with open(csv_path, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                company_name = cleaned_row.get('company_name', cleaned_row.get('name', ''))
                if company_name:
                    cleaned_row['company_name'] = company_name
                    cleaned_row.setdefault('legacy_company_name', company_name)
                lookup_key = (
                    cleaned_row.get('sha1')
                    or cleaned_row.get('sha1_name')
                    or cleaned_row.get('document_id')
                    or cleaned_row.get('filename')
                    or cleaned_row.get('source')
                )
                if not lookup_key:
                    raise ValueError(
                        "Metadata CSV requires one of: sha1, sha1_name, document_id, filename, or source"
                    )
                metadata_lookup[Path(str(lookup_key)).stem] = cleaned_row
...
class JsonReportProcessor:
    ...
    def assemble_metainfo(self, data):
        ...
        # Add legacy or generic CSV metadata if available.
        ...
        return normalize_document_metadata(
            metainfo,
            source_identifier=data['origin']['filename'],
        )
