...
class EvidenceReference(BaseModel):
    document_id: str = Field(description="Stable identifier of the retrieved document")
    page_number: int = Field(description="One-based page number in that document")


class GenericRAGPrompt:
    instruction = """
你是一个通用企业文档RAG问答系统。
只能根据检索上下文作答，不得使用上下文之外的事实。
上下文可能来自多个文档；引用证据时必须同时给出原样的 document_id 和 page_number。
如果上下文不足以支持答案，返回 'N/A'，并保持 relevant_sources 为空。
不要因为文档所属行业、组织或文件类型不同而改变这些规则。
"""
...
'''
    ...
    class TextAnswerSchema(BaseModel):
        step_by_step_analysis: str = Field(description="基于给定证据的分析过程")
        reasoning_summary: str = Field(description="简要说明答案如何由证据支持")
        relevant_pages: List[int] = Field(default_factory=list, description="Legacy compatibility field; generic mode may leave it empty")
        relevant_sources: List[EvidenceReference] = Field(default_factory=list, description="Only document/page pairs directly used in the answer")
        final_answer: Union[str, Literal["N/A"]] = Field(description="Direct answer, or 'N/A' when evidence is insufficient")

    class NumberAnswerSchema(BaseModel):
        step_by_step_analysis: str = Field(description="基于给定证据的分析过程")
        reasoning_summary: str = Field(description="简要说明答案如何由证据支持")
        relevant_pages: List[int] = Field(default_factory=list, description="Legacy compatibility field; generic mode may leave it empty")
        relevant_sources: List[EvidenceReference] = Field(default_factory=list, description="Only document/page pairs directly used in the answer")
        final_answer: Union[float, int, Literal["N/A"]] = Field(description="Exact numeric answer, or 'N/A' when evidence is insufficient")

    class BooleanAnswerSchema(BaseModel):
        step_by_step_analysis: str = Field(description="基于给定证据的分析过程")
        reasoning_summary: str = Field(description="简要说明答案如何由证据支持")
        relevant_pages: List[int] = Field(default_factory=list, description="Legacy compatibility field; generic mode may leave it empty")
        relevant_sources: List[EvidenceReference] = Field(default_factory=list, description="Only document/page pairs directly used in the answer")
        final_answer: Union[bool, Literal["N/A"]] = Field(description="True, False, or 'N/A' when evidence is insufficient")

    class NamesAnswerSchema(BaseModel):
        step_by_step_analysis: str = Field(description="基于给定证据的分析过程")
        reasoning_summary: str = Field(description="简要说明答案如何由证据支持")
        relevant_pages: List[int] = Field(default_factory=list, description="Legacy compatibility field; generic mode may leave it empty")
        relevant_sources: List[EvidenceReference] = Field(default_factory=list, description="Only document/page pairs directly used in the answer")
        final_answer: Union[List[str], Literal["N/A"]] = Field(description="Exact list answer, or 'N/A' when evidence is insufficient")
...
如果上下文没有足够信息判断True或False，则返回'N/A'。
""")
